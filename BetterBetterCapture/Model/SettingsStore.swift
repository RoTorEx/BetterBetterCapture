...
enum MicrophoneGain: String, CaseIterable, Identifiable {
    ...
    var linearGain: Double {
        ...
        case .off:
            return 1.0
        case .boost10:
            return pow(10.0, 10.0 / 20.0)
        case .boost20:
            return pow(10.0, 20.0 / 20.0)
        case .boost30:
            return pow(10.0, 30.0 / 20.0)
        }
    }
}
...
final class SettingsStore {
    ...
    var recordAudioOnly: Bool {
        get {
            access(keyPath: \.recordAudioOnly)
            return defaults.bool(forKey: "recordAudioOnly")
        }
        set {
            withMutation(keyPath: \.recordAudioOnly) {
                defaults.set(newValue, forKey: "recordAudioOnly")
            }
        }
    }
    ...
    var microphoneGain: MicrophoneGain {
        get {
            MicrophoneGain(rawValue: microphoneGainRaw) ?? .auto
        }
        set {
            microphoneGainRaw = newValue.rawValue
        }
    }
    ...
    var showBetterBetterCapture: Bool {
        get {
            access(keyPath: \.showBetterBetterCapture)
            return defaults.object(forKey: "showBetterBetterCapture") as? Bool ?? false
        }
        set {
            withMutation(keyPath: \.showBetterBetterCapture) {
                defaults.set(newValue, forKey: "showBetterBetterCapture")
            }
        }
    }
    ...
    /// The default output directory (Movies/BetterBetterCapture)
    var defaultOutputDirectory: URL {
        URL.homeDirectory.appending(path: "Movies/BetterBetterCapture")
    }
    ...
    private var microphoneGainRaw: String {
        get {
            access(keyPath: \.microphoneGainRaw)
            return defaults.string(forKey: "microphoneGain") ?? MicrophoneGain.auto.rawValue
        }
        set {
