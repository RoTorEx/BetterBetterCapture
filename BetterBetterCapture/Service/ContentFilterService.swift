...
final class ContentFilterService {
    ...
    private let logger = Logger(subsystem: Bundle.main.bundleIdentifier ?? "BetterBetterCapture", category: "ContentFilterService")
    ...
    func applySettings(to filter: SCContentFilter, settings: SettingsStore) async throws -> SCContentFilter {
        ...
        // If both wallpaper and dock are shown and BetterBetterCapture is shown, no need to rebuild the filter
        if settings.showWallpaper && settings.showDock && settings.showBetterBetterCapture {
        ...
        for window in availableWindows {
            ...
            // Exclude BetterBetterCapture's own windows if showBetterBetterCapture is false
            if !settings.showBetterBetterCapture && bundleID == Bundle.main.bundleIdentifier {
                excludedWindows.append(window)
                logger.debug("Excluding BetterBetterCapture window: \(windowTitle)")
                continue
            }
